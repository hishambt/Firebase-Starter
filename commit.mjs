...
exec('git branch --show-current', (err, stdout, stderr) => {
	const branchSplitted = stdout.split('-');
	const jiraTicketId = [branchSplitted[0], branchSplitted[1]].join('-');
	startPrompt(jiraTicketId);
});

function startPrompt(jiraTicketId) {
	prompt([
		{
			type: 'list',
			name: 'type',
		...
		const scope = answers.specificScope
